...
We want to update the document and ensure an update hasn't been made by other client:
...
If we don't have the version number (it generally comes through the payload of POSTs or PUTs), we can request it along with search results:
...
When you have too many shards and not enough documents, things may not work the way you expect. The IDF is computed per shard, so if you get unexpected search results and you have just few documents, you might try putting everything in just 1 shard.
...
**NOTE**: Why `routing == 1`? When using parent/child relationships, all documents must be stored in the same shard.
...
Or for all children whose parents match "Wars" (there is only 1 parent in this example, Star Wars...):
...
*NOTE*: Parent-child relationships may come with a performance hit both to maintain the index and run the queries, so use them wisely.
